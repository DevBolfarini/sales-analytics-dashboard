...
from database.crud import conectar  # noqa: E402
...
def rodar_etl_carga() -> None:
    """
    Executa o processo de Carga (ETL) utilizando inserção em massa
    (Bulk Insert)
    ...
    try:
        ...
        logging.info(
            f"Extração: {len(df_vendas_historico)} registros lidos do "
            "arquivo CSV."
        )

        # === 2. TRANSFORMAR (Transform) ===
        # Tratamento de dados nulos (Missing Values)
        df_vendas_historico['Quantidade'] = (
            df_vendas_historico['Quantidade'].fillna(1)
        )
        df_vendas_historico['Vendedor Nome'] = (
            df_vendas_historico['Vendedor Nome'].fillna("Desconhecido")
        )
        ...
        logging.info(
            "Transformação concluída: dados limpos e colunas padronizadas."
        )
        ...
        logging.info(
            f"Carga Concluída! {len(df_vendas_historico)} registros "
